...
### Added
- **Run (▶) button on every prompt row in the sidebar tree.** A new inline play icon sits next to the existing copy icon and dispatches the prompt's content according to the new `promptPocket.runAction` setting. By default (`copilotChat`) it opens the editor's AI chat in **Agent mode** with the prompt prefilled via `workbench.action.chat.open({ query, mode: 'agent' })` — this works for both Copilot Chat (VS Code 1.95+) and Cursor's chat panel, since both honor the `mode` hint. If no AI chat is registered (e.g. VSCodium or VS Code without Copilot) it gracefully falls back to copying the prompt to the clipboard with a warning toast. Also accessible via right-click → **Run Prompt**.
- **New `promptPocket.runAction` setting** controlling what the Run button does. Four options: `copilotChat` (default, send to Copilot Chat with clipboard fallback), `insertAtCursor` (insert at cursor in the active editor, replacing any selection, with clipboard fallback when no editor is active), `terminal` (send to a dedicated, reusable "Prompt Pocket" integrated terminal — no auto-Enter), and `clipboard` (same as the Copy button).
...
- **The copy / clipboard pattern that previously lived in three separate command handlers (`copyPrompt`, `copyPromptFromPalette`, `search`) is now a single shared `copyPromptContentWithFeedback` helper** in the new `src/runActions.ts` module. Behavior is unchanged for end users — same normalization, same toast — but adding new copy paths (or new Run actions that fall back to clipboard) no longer requires duplicating the pattern. `normalizeClipboardContent` moved out of `src/extension.ts` into `src/runActions.ts` along with it.
