...
### Added
- **Run (▶) button on every prompt row in the sidebar tree.** A new inline play icon sits next to the existing copy icon and dispatches the prompt's content according to the new `promptPocket.runAction` setting. By default (`copilotChat`) it opens VS Code Copilot Chat with the prompt prefilled via `workbench.action.chat.open`; if Copilot isn't installed it gracefully falls back to copying the prompt to the clipboard with a warning toast so Cursor / VSCodium / VS Code-without-Copilot users get a usable outcome. Also accessible via right-click → **Run Prompt**.
