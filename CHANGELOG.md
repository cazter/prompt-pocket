...
### Added
- **Run (▶) button on prompt rows in the main panel.** Previously Run was only available from the sidebar tree (inline icon + context menu); it's now next to the Copy button on every prompt row in the editor panel too, dispatching through the same `promptPocket.runAction` setting and showing the same outcome toast.
...
### Added
- **Run (▶) button for sidebar prompts.** Each prompt row now has an inline Run action, also available from the context menu, that dispatches the prompt through the new `promptPocket.runAction` setting.
- **Configurable run destinations.** `promptPocket.runAction` supports `copilotChat` (default, opens AI chat in Agent mode with clipboard fallback), `insertAtCursor`, `terminal`, and `clipboard`.

### Changed
- **Sidebar tree click now opens a prompt for editing instead of copying it.** Copy remains one click away through the inline copy icon, context menu, Command Palette copy flows, search, and the editor panel's Enter-to-copy behavior.
- **Prompt-content editing now live-autosaves in a normal markdown tab.** The editor opens in the active column, reuses the existing tab for repeat clicks, syncs changes back to Prompt Pocket as you type, and no longer shows the old save-on-close prompt.
- Consolidated sidebar, Command Palette, and search copy paths behind the shared `copyPromptContentWithFeedback` helper in `src/runActions.ts`, preserving the existing clipboard normalization and notification behavior.
...
[0.2.3]: https://github.com/cazter/prompt-pocket/releases/tag/v0.2.3
